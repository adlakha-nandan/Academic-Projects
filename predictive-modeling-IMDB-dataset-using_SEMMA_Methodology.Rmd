---
title: "Predictive Modeling on IMDB Dataset: using SEMMA Methodology"
author: "Nandan Adlakha, Abhiroop Raja, Sagar Thorat, Jonathan Cazar"
date: "4/21/2021"
output:
  html_document: 
    toc: yes
...
Team 4 Uconn Consultancy’s IMDb Dataset Predictive Modeling is about predicting success rate of the movie, essentially the IMDb Score, based on movie attributes like budget, genre, cast popularity, etc. The data of Movies all over the world are scraped from the IMDb website and after using unsupervised learning techniques for data exploration and pre-processing, the cleaned and balanced data is fed to our analytics solution model. The solution model is built using sophisticated predictive modeling techniques which involves continuous learning of past data, and building complex formulas and algorithms using all necessary possible movie attributes and provides high predictive capability to determine the IMDb score the movie is likely to have. With unknown data, the model could predict on average within 1.6% of the actual IMDb score. Along with the aim to predict a successful high-rated movie, the project is also able to determine who and what contributes towards that and will benefit moviemakers in making appropriate casting choices and movie selection based on genres.
...
In the past, many Production Houses in the entertainment industry relied solely on historic data performance to decide if they wanted to invest in a movie. In 1992, director James Cameron’s box office track record and originality of movies alone led Fox Studios to hire him in a 5-year deal worth $500 million. (Weinraub 1992) However with the boom of the internet and alternative entertainment options, the recipe for a successful movie has become much more complex. Multiple facets of talent are accessible to the general public and the popularity of a celebrity is determined by the number of followers, the number of likes they have on social media websites like Twitter, Facebook, Instagram, etc. Additionally, people have become aware of correlations between movie quality and movie budget and have viewing preferences for certain genres. Studios want to tap into this social feedback and make investment decisions in talent and movies based on it as well. Many movies are greenlit now-a-days and are guaranteed a certain success because a popular actor or director is headlining them. Moreover, with the deluge of films, and shortage of time, most people rely on analytics from platforms like IMDb, rotten tomatoes, etc to watch top-rated movies. Therefore, movie studios not only want to make popular movies for immediate box-office returns but also aim to own a high-rated product for continued returns via post-theatrical screening mediums. Using predictive modeling techniques, We at Team 4 UConn Consultancy try to find insights from the scraped IMDb data and help studios with actionable recommendations on cast and crew choices, budgeting and genre selection and with our analytics capability aim to predict a score for the movie success on a scale of 10 (0-low, 10-high) .
...
After running several iterations of our selected model types, Team 4 UConn Consultancy recommends that movie studios should recruit popular directors and assemble a cast that is popular to the demographic.  Our conclusion to recruit popular directors and cast members was reached based on our highest contributing predictors, such as our director facebook likes and cast facebook likes columns. 
	Since the audience and film reviewers typically relate higher budget movies with optimal quality, our next recommendation to movie studios which are heavily investing in movie production, is to make their budget information accessible to the targeted audience and film reviewers. 
	Our final recommendation would be to advise movie studios to produce more Drama movies because this genre generated higher ratings. Also, Drama films generally tend to be longer in duration, which was another contributing factor to the higher movie scores. Overall, for a movie to be successful in terms of IMDB ratings, movie studios should focus on recruiting popular directors and cast members, market their budget information, and produce more Drama films.
